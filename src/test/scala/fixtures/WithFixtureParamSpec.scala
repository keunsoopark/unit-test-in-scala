...
// As using FixtureAnyFlatSpec, you can make Load Fixture easier than the ones in ScalaLoanFixtureSpec.scala
class WithFixtureParamSpec extends FixtureAnyFlatSpec with Matchers {
  ...
  // need this to override "withFixture" method...
  case class FixtureParam(currencies: List[Currency])

  behavior of "Testing Random Currencies with withFixture"
  it should "have totalCost of Dollars more than $10" in { f => // "f" for Fixture (WithFixtureParamSpec class itself)
//    info(f.currencies.size.toString)
  ...
  it should "not have any currency with code INR or NZD" in { f =>
    f.currencies should contain noneOf ("IZR", "NZD")
  }
}
